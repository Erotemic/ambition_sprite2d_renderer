#!/usr/bin/env python3
"""Build the director-only paper-doll rig used by the vanity card.
...
Usage::
    ...
    uv run python scripts/build_director_vanity_rig.py build
    uv run python scripts/build_director_vanity_rig.py build --fresh
    uv run python scripts/build_director_vanity_rig.py validate
    uv run python scripts/build_director_vanity_rig.py preview

Open the result in the editor with::

    uv run --extra gui python -m ambition_sprite2d_renderer.gui \
        assets/rigs/director_vanity/director_vanity.rig.json
"""
...
SVG_PATH = ROOT / "assets/director-rig-labels-joints.svg"
RIG_DIR = ROOT / "assets/rigs/director_vanity"
RIG_PATH = RIG_DIR / "director_vanity.rig.json"
SCRATCH = ROOT / "agent-scratch/director_vanity"
VIEW_LABEL = "Director - Side West"
...
def validate_svg() -> None:
    ...
    if root.get("data-asset-role") != "vanity-card-animation":
        raise ValueError("director SVG must declare data-asset-role=vanity-card-animation")

    view = next((elem for elem in root.iter() if elem.get(INK_LABEL) == VIEW_LABEL), None)
    if view is None:
        raise ValueError(f"director SVG is missing view layer {VIEW_LABEL!r}")
    if view.get(INK_GROUPMODE) != "layer":
        raise ValueError("director view must be an Inkscape layer")
    ...
    if len(part_names) != len(set(part_names)):
        raise ValueError("director SVG contains duplicate data-rig-part names")
    if set(part_names) != EXPECTED_PARTS:
        raise ValueError(
            "director SVG part contract differs: "
    ...
    if len(joint_names) != len(set(joint_names)):
        raise ValueError("director SVG contains duplicate data-rig-joint names")
    if set(joint_names) != EXPECTED_JOINTS:
        raise ValueError(
            "director SVG joint contract differs: "
...
def cmd_validate(args: argparse.Namespace) -> None:
    ...
    for clip, frames, _duration in doc.rows():
        ...
        for image in samples:
            ...
            x += image.width
        out = SCRATCH / f"director_vanity_{clip}_samples.png"
        strip.save(out)
        print(out)
