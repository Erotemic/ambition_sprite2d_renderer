...
DIRECTOR_RIG_PATH = ROOT / "assets" / "rigs" / "director_vanity" / "director_vanity.rig.json"
ROBOT_RIG_PATH = player_robot_v3_target.RIG_PATH
GAMEPAD_SVG_PATH = ROOT / "assets" / "gamepad-draft.svg"
DEFAULT_OUT_DIR = ROOT / "agent-scratch" / "director_vanity_dialog"
...
class TimelineState:
    ...
    director_reach_strength: float
    director_body_lean: float
    director_head_tilt: float
...
def ensure_rigs_exist() -> None:
    checks = [
        (
            DIRECTOR_RIG_PATH,
            [sys.executable, str(ROOT / "scripts" / "build_director_vanity_rig.py"), "build"],
        ),
        (
            ROBOT_RIG_PATH,
...
def timeline_state(frame_index: int) -> TimelineState:
    ...
    if frame_index <= 11:
        ...
        return TimelineState(
            frame_index, robot_x, DIRECTOR_X,
    ...
    if frame_index <= 23:
        ...
        return TimelineState(
            frame_index, ROBOT_X_END, DIRECTOR_X,
    ...
    # 24-35: director reacts, leaning down slightly, with enough pause for the beat.
    if frame_index <= 35:
    ...
    # 44-57: the robot lets go and backs off; the director draws the prop in.
    #
    ...
    # the beat that used to follow opened at 0.04, which snapped the director's
    ...
    if frame_index <= 57:
        ...
        return TimelineState(
            frame_index, _lerp(ROBOT_X_END, ROBOT_X_BACK, t), DIRECTOR_X,
    ...
    # 58-69: the director looks DOWN at the prop and contemplates it. This is the
    ...
    #
    #  and the number is PIVOT-DEPENDENT, so re-sweep it whenever the director's
    ...
    if frame_index <= 69:
        ...
        return TimelineState(
            frame_index, ROBOT_X_BACK, DIRECTOR_X,
    ...
    director_bob = math.sin(linger * 2.0 * math.pi)
    return TimelineState(
        frame_index, ROBOT_X_BACK, DIRECTOR_X,
        "idle", (0.46 + 0.10 * linger) % 1.0,
        "vanity_receive", 0.90 + 0.04 * linger,
        (408.0, 190.0 - 1.2 * director_bob), 22.0, False, True,
        0.0, 0.34, 0.10, -36.0, 12.0 * droop,
        "director", "I made this.", True,
    )
...
def compose_actors(
    state: TimelineState,
    director_doc: RigDocument,
    director_hands_doc: RigDocument,
    director_style: ActorStyle,
    ...
    director_targets = None
    if state.director_reach_strength > 0.02:
        director_targets = {
    ...
    director = make_actor_for_state(
        director_doc,
        director_hands_doc,
        director_style,
        name="director",
        clip_name=state.director_clip,
        t=state.director_t,
        x_center=state.director_x,
        arm_targets_screen=director_targets,
        arm_pitches={"far": 166.0, "near": 174.0},
        arm_bends={"far": -1.0, "near": 1.0},
        body_pose={
            "torso": -18.0 * state.director_body_lean,
            "head": state.director_head_tilt,
            "pelvis": 1.5 * state.director_body_lean,
        },
    )
    return robot, director
...
def render_frame(
    ...
    director_doc: RigDocument,
    director_hands_doc: RigDocument,
    director_body_doc: RigDocument,
    director_near_arm_doc: RigDocument,
    director_style: ActorStyle,
    robot_doc: RigDocument,
    robot_hands_doc: RigDocument,
    robot_style: ActorStyle,
    ...
    director_body_layer, _ = render_layer(
        director_body_doc, state.director_clip, state.director_t, director.world, director.params,
        director_style, state.director_x, FLOOR_Y, mirrored=False
    )
    director_near_arm_layer, _ = render_layer(
        director_near_arm_doc, state.director_clip, state.director_t, director.world, director.params,
        director_style, state.director_x, FLOOR_Y, mirrored=False
    )

    canvas.alpha_composite(robot.full_layer.image, robot.full_layer.paste_xy)
    canvas.alpha_composite(director_body_layer.image, director_body_layer.paste_xy)
    draw_rotated_prop(canvas, prop, state.gamepad_center, state.gamepad_angle)

    # Keep the gamepad between the director's far and near limbs: the body layer
    ...
    # where the director's near arm vanished entirely.
    canvas.alpha_composite(director_near_arm_layer.image, director_near_arm_layer.paste_xy)
    ...
    elif state.bubble_speaker == "director" and state.bubble_text:
        draw_speech_bubble(
            canvas,
            state.bubble_text,
            director.head_anchor(),
            (388, 24),
        )
    return canvas
...
def save_storyboards(frames: Sequence[Image.Image], out_dir: Path) -> List[Path]:
    ...
    strip_path = out_dir / "director_vanity_dialog_strip.png"
    ...
    board_path = out_dir / "director_vanity_dialog_storyboard.png"
...
def build_dialog_sequence(out_dir: Path) -> List[Path]:
    ...
    director_doc = RigDocument.load(DIRECTOR_RIG_PATH)
    robot_doc = player_robot_v3_target.load_doc()
    director_hands_doc = hand_only_doc(director_doc)
    robot_hands_doc = hand_only_doc(robot_doc)
    director_near_arm_doc = parts_include_doc(director_doc, ["near_arm_u", "near_arm_l", "near_sleeve", "near_hand"])
    director_body_doc = parts_exclude_doc(director_doc, ["near_arm_u", "near_arm_l", "near_sleeve", "near_hand"])
    director_style = calculate_style(director_doc, "vanity_idle", DIRECTOR_TARGET_HEIGHT)
    ...
    gif_path = out_dir / "director_vanity_dialog.gif"
    ...
    webp_path = out_dir / "director_vanity_dialog.webp"
    frames[0].save(
        webp_path,
        save_all=True,
    ...
    manifest_path = out_dir / "director_vanity_dialog_manifest.txt"
    manifest_path.write_text(
        "\n".join(
            [
                "Director vanity dialog animation",
                f"frames={ANIMATION_FRAMES}",
                f"duration_ms={FRAME_DURATION_MS}",
                f"director_rig={DIRECTOR_RIG_PATH.relative_to(ROOT)}",
                ...
                "director path: idle -> look/question -> IK reach down -> take -> contemplate -> claim",
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Render the polished Director vanity-card animation.")
