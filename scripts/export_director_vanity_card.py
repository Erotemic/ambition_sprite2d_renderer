"""Bake the director vanity-card animation into placements the engine can play.
...
Usage::
    ...
    uv run python scripts/export_director_vanity_card.py
    uv run python scripts/export_director_vanity_card.py --verify
"""
...
# The director's near-side arm draws OVER the prop; everything else on the director
...
Z_ROBOT = 0
Z_DIRECTOR_BODY = 1000
Z_PROP = 2000
Z_DIRECTOR_NEAR_ARM = 3000
...
def bake(verify: bool) -> Tuple[dict, PartAtlas]:
    ...
    director_doc = RigDocument.load(dialog.DIRECTOR_RIG_PATH)
    robot_doc = player_robot_v3_target.load_doc()
    director_hands_doc = dialog.hand_only_doc(director_doc)
    robot_hands_doc = dialog.hand_only_doc(robot_doc)
    director_style = dialog.calculate_style(director_doc, "vanity_idle", dialog.DIRECTOR_TARGET_HEIGHT)
    ...
    # the director's hands four times too far apart.
    ...
    for frame_index in range(dialog.ANIMATION_FRAMES):
        ...
        robot, director = dialog.compose_actors(
            state,
            director_doc,
            director_hands_doc,
            director_style,
        ...
        draws += part_draws(
            atlas, "director", director, director_doc, Z_DIRECTOR_BODY, without=NEAR_ARM_PARTS
        )
        draws.append(prop_draw(atlas, prop, state))
        draws += part_draws(
            atlas, "director", director, director_doc, Z_DIRECTOR_NEAR_ARM, only=NEAR_ARM_PARTS
        )
        ...
        if state.bubble_text and state.bubble_speaker:
            anchor = (
                robot.head_anchor() if state.bubble_speaker == "robot" else director.head_anchor()
            )
...
def verify_frame(
    ...
    try:
        reference = dialog.render_frame(
            ...
            director_doc,
            director_hands_doc,
            dialog.parts_exclude_doc(director_doc, NEAR_ARM_PARTS),
            dialog.parts_include_doc(director_doc, NEAR_ARM_PARTS),
            director_style,
            robot_doc,
            robot_hands_doc,
            robot_style,
...
def write_manifest(
    ...
    lines: List[str] = []
    lines.append("// GENERATED by tools/ambition_sprite2d_renderer/scripts/export_director_vanity_card.py")
