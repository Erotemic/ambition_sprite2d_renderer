...
SVG = ROOT / "assets/director-rig-labels-joints.svg"
RIG = ROOT / "assets/rigs/director_vanity/director_vanity.rig.json"
INK_LABEL = "{http://www.inkscape.org/namespaces/inkscape}label"


def test_director_vanity_asset_stays_outside_game_registry():
    assert RIG.exists()
    assert "targets/characters/rigged" not in RIG.as_posix()
    data = json.loads(RIG.read_text(encoding="utf8"))
    assert data["name"] == "director_vanity"
    assert data["features"]["registered_game_target"] is False
    assert "director_vanity" not in rigged_targets.TARGETS
...
def test_director_vanity_svg_contract():
    root = ET.fromstring(SVG.read_bytes())
    view = next(elem for elem in root.iter() if elem.get(INK_LABEL) == "Director - Side West")
...
def test_director_vanity_rig_loads_and_solves():
