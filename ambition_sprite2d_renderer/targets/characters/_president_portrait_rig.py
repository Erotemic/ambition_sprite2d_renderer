"""Explicit joint skeleton for president_portrait (faithful extraction).

Like ``_viking_warrior_rig`` and ``_colonial_statesman_rig``, this lifts the
character's implicit joint computation out of the paint pass into a declared,
poseable rig — so the animation lives on named joints that can be edited and
sampled, not buried in ``_render_frame``. Nothing about the drawn result
changes: ``_render_frame`` reads every limb/head anchor from :func:`evaluate`
instead of recomputing it.

The president uses the position-shift model (NOT the pirate's rotate-around-joint
...
def evaluate(pose, work_w: float, work_h: float) -> PresidentPortraitJoints:
    ...
    return PresidentPortraitJoints(
        root=root,
        body_ang=body_ang,
        head_root=P(-2, -246),
...
__all__ = ["PresidentPortraitJoints", "evaluate"]
