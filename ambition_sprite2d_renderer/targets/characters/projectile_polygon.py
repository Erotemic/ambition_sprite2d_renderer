...
⛔ **He does not swing, so nothing here is measured like a swing.** His danger
leaves the weapon: the axis is the BARREL, read off a `cannon` bone parented to
his head, and every effect is built from the muzzle OUTWARD rather than along
the arc of a limb. Aiming is one number per frame (head rotation), and recoil is
what makes a shot read as a shot rather than a laser pointer — so the poses
drive the body backwards along the barrel every time he fires.
"""
...
def render(out_dir: str | Path, **opts):
    ...
    outputs = build_sheet(
        ...
        # ⛔ `authored`, NOT `art`: a beam is nine barrel-lengths long, so a body
        # measured per-pose would hand him a hurtbox the size of his own shot.
