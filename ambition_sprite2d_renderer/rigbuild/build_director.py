"""Assemble the Director's character rig SVG from the authored art source."""
...
from . import pen_director

REPO = Path(__file__).resolve().parents[2]
SRC = REPO / "assets/director-rig-labels-joints.svg"
DST = REPO / "ambition_sprite2d_renderer/data/characters/director/director.svg"
...
def main() -> None:
    ...
    view = by_id["view-director-side-west"]
    ...
    pen.text = "\n        "
    for eid, d, style in pen_director.PATHS:
    ...
    parts = bc.collect_parts(root, "view-director-side-west")

    meta_text, markers_text = bc.build(
        view_id="view-director-side-west",
        source_layer="view-director-side-west",
        source_label="Director - Side West",
        facing="west",
        catalog_view="side",
        joints=joints,
        parts=parts,
    ...
    # marker layer: one rest directority, still editable in Inkscape.
    by_id["rig-joints"].getparent().remove(by_id["rig-joints"])

    install_managed_block(root, meta_text, markers_text)

    root.set("sodipodi:docname".replace("sodipodi:", f"{{http://sodipodi.sourceforge.net/DTD/sodipodi-0.dtd}}"), "director.svg")
