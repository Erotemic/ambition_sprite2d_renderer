...
def doc_to_python(doc: RigDocument) -> str:
    ...
    a("    if SS == 1:")
    a("        return img")
    a("    # ⛔⛔ NOT `img.resize(..., LANCZOS)`, which is what stood here.")
    a("    # A Lanczos kernel has negative lobes, so reducing a supersampled")
    a("    # silhouette laid over transparency leaves faint non-zero-alpha")
    a("    # pixels several texels outside it — the pale halo. The interpreter")
    a("    # stopped doing that; a generated target that kept doing it renders")
    a("    # a DIFFERENT image from the document it was generated from.")
    a("    return resize_transparent_sprite(")
    a("        img,")
    a("        (FRAME_W * RENDER_SCALE, FRAME_H * RENDER_SCALE),")
    a("        reducing_gap=3.0,")
    a("    )")
    a("")
    a("")
